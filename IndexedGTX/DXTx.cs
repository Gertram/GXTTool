...
namespace IndexedGTX
{
    public static class DXTx
    {
        ...
        public static byte[] Decompress(BinaryReader reader, int dataSize,int width,int height,SceGxmTextureBaseFormat format)
        {
            byte[] pixelData = new byte[dataSize * 8];
            ...
            for (int y = 0; y < height; y += 4)
            {
                ...
                {
                    byte[] decodedBlock = DecompressDxtBlock(reader, format);
